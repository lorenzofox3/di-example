...
export const composeP = (fns) => (args) => fns.reduceRight((x, f) => x.then(f), Promise.resolve(args));

export const map = (fn) => (functor) => functor.map(fn);

export const mapEntries = (fn) => (target) => Object.fromEntries(Object.entries(target).map(fn));
